...
import { CreateParcelStep2 } from "../../utils/mutations/accountMutations"
...
export default function SenderReceiverDetails() {
  ...
  const [senderName, setSenderName] = useState(deliveryDetails.senderName || "Qamardeen Abdul Malik")
  const [senderPhone, setSenderPhone] = useState(deliveryDetails.senderPhone || "07033484845")
  const [receiverName, setReceiverName] = useState(deliveryDetails.receiverName || "")
  const [receiverPhone, setReceiverPhone] = useState(deliveryDetails.receiverPhone || "")
  ...
  const { mutate: submitStep2, isPending: isSubmittingStep2 } = useMutation({
    mutationFn: (data: any) => CreateParcelStep2({ id: parcelId, data, token: token! }),
    onSuccess: (response) => {
      ...
      Toast.show({
        ...
        text2: "Proceed to parcel details",
      });
    ...
    onError: (error) => {
      ...
      Toast.show({
        ...
        text2: "Please check sender/receiver details",
      });
    },
  });
