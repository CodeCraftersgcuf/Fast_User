...
export default function WalletScreen() {
  ...
  const handlePlaceWithdrawal = () => {
    ...
    setTimeout(() => {
      ...
      if (Math.random() > 0.1) {
        setConfirmationStatus("success")
        // Add transaction to history
        const amount = Number.parseInt(withdrawAmount, 10) || 0
        const newTransaction: Transaction = {
          id: (transactions.length + 1).toString(),
          type: "withdrawal",
          amount,
  ...
  const renderTransactionItem = ({ item }: { item: Transaction }) => (
    <View style={styles.transactionItem}>
      <View style={styles.transactionIconContainer}>
        <Icon name={item.type === "topup" ? "arrow-down" : "arrow-up"} size={24} color="#800080" />
      </View>
      <View style={styles.transactionInfo}>
        <Text style={styles.transactionType}>{item.type === "topup" ? "Deposit" : "Withdrawal"}</Text>
        <Text style={styles.transactionDate}>{item.date}</Text>
      </View>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* Topup Modal */}
      <Modal visible={activeModal === "topup"} transparent={true} animationType="slide" onRequestClose={closeModal}>
        <SafeAreaView style={styles.modalContainer}>
          <View style={styles.modalHeader}>
            <TouchableOpacity onPress={closeModal} style={styles.modalBackButton}>
              <Icon name="chevron-back" size={24} color="#000000" />
      ...
      <Modal Modal visible={activeModal === "withdraw"
        ...
        <SafeAreaView style={styles.modalContainer}>
          ...
          <View style={styles.modalContent}>
            ...
            <View style={styles.formGroup}>
              <Text style={styles.formLabel}>Bank Name</Text>
              <TextInput style={styles.formInput} placeholder="Bank Name" value={bankName} onChangeText={setBankName} />
            </View>
            ...
            <TouchableOpacity style={styles.checkboxContainer} onPress={() => setSaveBankDetails(!saveBankDetails)}>
              <View style={[styles.checkbox, saveBankDetails && styles.checkboxChecked]}>
                {saveBankDetails && <Icon name="checkmark" size={16} color="#FFFFFF" />}
              </View>
              <Text style={styles.checkboxLabel}>Save Bank Details</Text>
          ...
          <View style={styles.confirmationContainer}>
            ...
            <View style={styles.confirmationIconContainer}>
              ...
              >
                ...
                >
                  <Icon name={confirmationStatus === "success" ? "checkmark" : "close"} size={40} color="#FFFFFF" />
                </View>
              </View>
            </View>
